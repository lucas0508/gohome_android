package com.qujiali.jiaogegongren.common.model.captchaview;
...
import static com.qujiali.jiaogegongren.common.model.captchaview.DrawHelperUtils.drawPartCircle;
...
/*        mMaskShadowPaint.setStrokeWidth(50);
    ...
    private void createCaptchaPath() {
       ...
       /* RectF oval = new RectF(mCaptchaX + gap, mCaptchaY - (gap), mCaptchaX + gap + gap * 2, mCaptchaY + (gap));
        mCaptchaPath.addArc(oval, 180,180);
        mCaptchaPath.lineTo(mCaptchaX + mCaptchaWidth, mCaptchaY);
        //凹的话，麻烦一点，要利用多次move
        mCaptchaPath.lineTo(mCaptchaX + mCaptchaWidth, mCaptchaY + gap);
        oval = new RectF(mCaptchaX + mCaptchaWidth - gap, mCaptchaY + gap, mCaptchaX + mCaptchaWidth + gap, mCaptchaY + gap + gap * 2);
        mCaptchaPath.addArc(oval, 90, 180);
        mCaptchaPath.moveTo(mCaptchaX + mCaptchaWidth, mCaptchaY + gap + gap * 2);
    ...
    public void setBitmap(SwipeCaptchaView swipeCaptchaView,int drawableId) {
        Bitmap bitmap = BitmapFactory.decodeResource(mContext.getResources(), drawableId);
        setBitmap(swipeCaptchaView,bitmap);
    }

    public void setBitmap(SwipeCaptchaView swipeCaptchaView, Bitmap bitmap) {
        swipeCaptchaView.setScaleType(ScaleType.CENTER_CROP);
